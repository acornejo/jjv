  ...
  var checkValidity = function (schema, object, name, required, options) {
    ...
    if (schema.hasOwnProperty('allOf')) {
      ...
      for (i = 0, len = schema.oneOf.length, count = 0; i < len; i++) {
        ...
        objerr = checkValidity(schema.oneOf[i], orig_object, name, required, options);
        if (!objerr) {
          count = count + 1;
          if (count > 1)
            break;
      ...
      for (i = 0, len = schema.anyOf.length; i < len; i++) {
        orig_object[name] = clone(object[name]);
        objerr = checkValidity(schema.anyOf[i], orig_object, name, required, options);
        if (!objerr) {
          object[name] = orig_object[name];
          return null;
        }
