...
(function () {
  ...
  var validateField = function () {
    ...
    d.notIn = function (v, p) {
      for (var i = 0; i < p.length; i++)
        if (v === p[i]) throw new Error('found in set');
    };
  ...
  var checkValidity = function (schema, object, name, required) {
    ...
    if (object.hasOwnProperty(name)) {
      ...
      if (schema.type === 'object' && schema.hasOwnProperty('properties')) {
        ...
        for (p in schema.properties) {
          objerr = checkValidity(schema.properties[p], object[name], p, schema.required);
          if (objerr !== null) {
            objerrs[p] = objerr;
            malformed = true;
          }
        ...
        for (i = 0; i<object[name].length; i++) {
          objerr = checkValidity(schema.items, object[name], i, schema.required);
          if (objerr !== null) {
            objerrs[i] = objerr;
            malformed = true;
          }
      ...
      objerrs = {};
      for (v in schema) {
        if (schema.hasOwnProperty(v) && v !== 'properties' && v !== 'items' && v !== 'type' && v !== 'meta' && v !== 'required' && v !== 'methods') {
  ...
  var schemaValidator = function (schema, object, options) {
    ...
    options = options || {};
    options.remove = options.hasOwnProperty('remove') ? options.remove : true;
    ...
    if (schema.hasOwnProperty('properties')) {
      ...
      properr = checkValidity(schema, root_object, root_key, [root_key]);
      if (properr !== null && properr.hasOwnProperty('schema'))
        validation = properr.schema;
      additional = checkExtra(schema, root_object, root_key, '', options.remove);
    }
  ...
  if (typeof exports !== 'undefined') {
    ...
    exports.addType = function (name, func) {
      enforceType[name] = func;
    };
    exports.addCheck = function (name, func) {
