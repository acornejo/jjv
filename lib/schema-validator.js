      ...
      if (typeof v === typeof p[0]) {
        ...
        if (typeof v === 'object') {
          for (i = 0, len = p.length; i < len; i++)
            if (JSON.stringify(v) === JSON.stringify(p[i]))
              return;
        } else {
          for (i = 0, len = p.length; i < len; i++)
            if (v === p[i])
  ...
  var checkValidity = function (schema, object, name, required, options) {
    ...
    if (schema.hasOwnProperty('allOf')) {
      ...
      for (i = 0, len = schema.allOf; i < len; i++) {
        objerr = checkValidity(schema.allOf[i], object, name, schema.allOf[i].required, options);
        if (objerr)
          return objerr;
      }
      ...
      for (i = 0, len = schema.oneOf, count = 0; i < len; i++) {
        objerr = checkValidity(schema.oneOf[i], object, name, schema.oneOf[i].required, options);
        if (!objerr) {
          if (count > 0)
            return {'oneOf': true};
          else
      ...
      for (i = 0, len = schema.anyOf; i < len; i++) {
        objerr = checkValidity(schema.anyOf[i], object, name, schema.anyOf[i].required, options);
        if (!objerr)
          return null;
      }
