...
(function () {
  ...
  var resolveURI = function (env, root_schema, uri) {
    ...
    if (hash_idx > 0) {
      ...
      if (!env.schema.hasOwnProperty(name)) {
        if (root_schema && root_schema.id !== name)
          return null;
      } else {
  ...
  var checkValidity = function (env, root_schema, schema, object_stack, options) {
    ...
    var i, len, count, orig_object = {}, refs, hasProp, hasPattern;

    var object = object_stack[object_stack.length-1].object, name = object_stack[object_stack.length-1].key, prop = object[name];
    ...
    if (!options.useCoerce && !options.useDefault && !options.removeAdditional) {
      ...
      if (schema.hasOwnProperty('not')) {
        objerr = checkValidity(env, root_schema, schema.not, object_stack, options);
        if (!objerr)
          return {'not': true};
      }
    } else {
      if (schema.hasOwnProperty('oneOf')) {
        for (i = 0, len = schema.oneOf.length, count = 0; i < len; i++) {
          ...
          objerr = checkValidity(env, root_schema, schema.oneOf[i], new_stack, options);
          if (!objerr) {
            count = count + 1;
            if (count > 1)
              break;
      ...
      if (schema.hasOwnProperty('anyOf')) {
        for (i = 0, len = schema.anyOf.length; i < len; i++) {
          new_stack = clone_stack(object_stack);
          objerr = checkValidity(env, root_schema, schema.anyOf[i], new_stack, options);
          if (!objerr) {
            copy_stack(new_stack, object_stack);
            break;
          }
      ...
      if (schema.hasOwnProperty('not')) {
        objerr = checkValidity(env, root_schema, schema.not, clone_stack(object_stack), options);
        if (!objerr)
          return {'not': true};
      }
    }
    ...
    for (v in schema) {
      if (schema.hasOwnProperty(v) && !handled.hasOwnProperty(v)) {
        if (v === 'format') {
          ...
          if (env.fieldValidate.hasOwnProperty(v) && !env.fieldValidate[v](prop, schema[v], schema, object_stack, options)) {
            objerrs[v] = true;
            malformed = true;
          }
        }
  ...
  Environment.prototype = {
    validate: function (name, object, options) {
      var refs, errors = null, object_stack = [{object: {'__root__': object}, key: '__root__'}];
