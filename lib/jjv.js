...
(function () {
  ...
  var checkValidity = function (env, schema_stack, object, name, options) {
    ...
    if (schema.hasOwnProperty('$ref')) {
      refs = resolveURI(env, schema_stack, schema.$ref);
      if (!refs)
    ...
    if (schema.hasOwnProperty('type')) {
      if (typeof schema.type === 'string') {
        if (env.coerceType.hasOwnProperty(schema.type))
    ...
    if (schema.hasOwnProperty('anyOf')) {
      for (i = 0, len = schema.anyOf.length; i < len; i++) {
        orig_object[name] = clone(prop);
        objerr = checkValidity(env, schema_stack.concat(schema.anyOf[i]), orig_object, name, options);
        if (!objerr) {
          object[name] = orig_object[name];
          break;
        }
    ...
    if (schema.hasOwnProperty('not')) {
      orig_object[name] = clone(prop);
      objerr = checkValidity(env, schema_stack.concat(schema.not), orig_object, name, options);
      if (!objerr)
        return {'not': true};
    }
