...
(function () {
  ...
  var validateField = {
    'readOnly': function (v, p, name, options) {
  ...
  var checkValidity = function (env, schema_name, schema, object, name, required, options) {
    ...
    if (schema.hasOwnProperty('$ref')) {
      if (schema.$ref  === '#') {
        return checkValidity(env, schema_name, env.schema[schema_name], object, name, required, options);
      } else if (schema.$ref.indexOf('#/definitions/') === 0) {
        def = schema.$ref.slice(14);
        if (!env.schema[schema_name].hasOwnProperty('definitions') || !env.schema[schema_name].definitions.hasOwnProperty(def))
          throw new Error('jjv: in property \'' + name + '\' couldn\'t resolve refence to \'' + def + '\'.');
        return checkValidity(env, schema_name, env.schema[schema_name].definitions[def], object, name, required, options);
      } else
        throw new Error('jjv: in property \'' + name + '\' $ref has an unsupported uri.');
  ...
  Environment.prototype = {
    validate: function (name, object, options) {
      if (!this.schema.hasOwnProperty(name))
        throw new Error('jjv: no schema named ' + name + ' is registered.');

      var schema = this.schema[name], errors, validation = null, additional = [], properr, root_key = '', root_object = {};
      ...
      properr = checkValidity(this, name, schema, root_object, root_key, [root_key], options);
      if (properr)
