...
(function () {
  ...
  var formatField = {
    ...
    'ipv6': function(v) {
      if (typeof v !== 'string' || !(/^::|^::1|^([a-fA-F0-9]{1,4}::?){1,7}([a-fA-F0-9]{1,4})$/.test(str)))
  ...
  var checkValidity = function (env, root_schema, schema, object, name, required, options) {
    var p, v, malformed = false, objerrs = {}, objerr, objreq, errors = {}, props, matched;
    ...
    if (object.hasOwnProperty(name)) {
      ...
      if (schema.hasOwnProperty('not')) {
        orig_object[name] = clone(object[name]);
        objerr = checkValidity(env, root_schema, schema.not, orig_object, name, required, options);
        if (!objerr)
          return {'not': true};
      }
      ...
      if (schema.hasOwnProperty('dependencies')) {
        for (p in schema.dependencies)
          if (schema.dependencies.hasOwnProperty(p)) {
            if (schema.dependencies[p] instanceof Array) {
              for (i = 0, len = schema.dependencies[p].length; i < len; i++)
                if (!object[name].hasOwnProperty(schema.dependencies[p][i]))
                  return {'dependencies': true};
            } else {
              objerr = checkValidity(env, root_schema, schema.dependencies[p], object, name, required, options);
              if (objerr)
                return objerr;
            }
          }
      ...
      if (schema.hasOwnProperty('patternProperties') || schema.hasOwnProperty('additionalProperties')) {
          ...
          while (i--) {
            ...
            for (p in schema.patternProperties) {
              if (schema.patternProperties.hasOwnProperty(p) && props[i].match(p)) {
                objerr = checkValidity(env, root_schema, schema.patternProperties[p], object[name], props[i], schema.required, options);
                if (objerr !== null) {
        ...
        if (options.removeAdditional && schema.additionalProperties !== true) {
          for (i = 0, len = props.length; i < len; i++)
            delete object[name][props[i]];
        } else
        if (schema.hasOwnProperty('additionalProperties')) {
          if (typeof schema.additionalProperties === 'boolean') {
            ...
            for (i = 0, len = props.length; i < len; i++) {
              objerr = checkValidity(env, root_schema, schema.additionalProperties, object[name], props[i], schema.required, options);
              if (objerr !== null) {
                objerrs[props[i]] = objerr;
                malformed = true;
              }
  ...
  Environment.prototype = {
    validate: function (name, object, options) {
      ...
      errors = checkValidity(this, refs[0], refs[1], root_object, root_key, [root_key], options);
