    ...
    for (v in schema) {
      if (schema.hasOwnProperty(v) && !handled.hasOwnProperty(v)) {
        if (v === 'format') {
          ...
          if (env.fieldValidate.hasOwnProperty(v) && !env.fieldValidate[v](prop && prop.hasOwnProperty('$data') ? resolveObjectRef(object_stack, prop.$data) : prop, schema[v], schema, object_stack, options)) {
            objerrs[v] = true;
            malformed = true;
          }
        }
